...
void txservice::remote::RemoteCcHandler::ObjectCommandOutside(
    const CcEntryAddr &cce_addr,
    TxCommand &obj_cmd,
    TxNumber tx_number,
    int64_t tx_term,
    ...
    ReadType read_type)
{
    ...
    send_msg.set_type(
        CcMessage::MessageType::CcMessage_MessageType_ApplyOutsideRequest);
    send_msg.set_tx_number(tx_number);
    send_msg.set_handler_addr(reinterpret_cast<uint64_t>(&hres));
    send_msg.set_tx_term(tx_term);
    send_msg.set_command_id(command_id);

    ApplyOutsideRequest *apply_outside = send_msg.mutable_apply_outside_req();
    assert(cce_addr.CcePtr() != 0);
    apply_outside->set_node_group_id(cce_addr.NodeGroupId());

    CceAddr_msg *cce_msg = apply_outside->mutable_cce_addr();
    cce_msg->set_cce_ptr(cce_addr.CcePtr());
    cce_msg->set_term(cce_addr.Term());
    cce_msg->set_core_id(cce_addr.CoreId());

    apply_outside->set_iso_level(ToRemoteType::ConvertIsolation(iso_level));
    apply_outside->set_protocol(ToRemoteType::ConvertProtocol(proto));
    apply_outside->set_apply_and_commit(commit);

    apply_outside->clear_cmd();
    std::string *cmd_str = apply_outside->mutable_cmd();
    obj_cmd.Serialize(*cmd_str);

    apply_outside->set_rec_ts(rec_ts);
    apply_outside->clear_record();
    if (read_type == ReadType::OutsideDeleted)
    {
        apply_outside->set_rec_status(RecordStatusType::DELETED);
    }
    else if (read_type == ReadType::OutsideNormal)
    {
        apply_outside->set_rec_status(RecordStatusType::NORMAL);
        (*rec)->Serialize(*apply_outside->mutable_record());
    }
    else
    {
        assert(false);
    ...
    // Notice: ObjectCommandOutside is different with ReadOutside,
    // it need the return value of SendRequest.
    stream_sender_.SendMessageToNg(cce_addr.NodeGroupId(), send_msg, &hres);
}
